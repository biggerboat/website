...
case 'promote':
	...
	foreach ( $userids as $id ) {
		...
		if ( $id == $currentuser->ID && ! $wp_roles->role_objects[ $_REQUEST['new_role'] ]->has_cap('promote_users')
...
<?php if ( $go_delete ) : ?>
	...
	<ul style="list-style:none;">
		...
		wp_dropdown_users( array( 'name' => 'reassign_user', 'exclude' => array_diff( $userids, array($currentuser->ID) ) ) ); ?></li>
	</ul></fieldset>
...
case 'doremove':
	...
	$update = 'remove';
 	foreach ( $userids as $id ) {
		$id = (int) $id;
		if ( $id == $currentuser->ID && !is_super_admin() ) {
